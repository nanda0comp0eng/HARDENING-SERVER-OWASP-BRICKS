...
// Initialize session and error message
session_start();
$sSuccessMsg = "";
    ...
    if (!$csrf_token || !validateCSRFToken($csrf_token)) {
        ...
        if (!trackLoginAttempts()) {
            ...
            if (empty($username)) {
                ...
                // Hash the password (recommend using password_hash in production)
                $hashed_pwd = $pwd;
                ...
                if (mysqli_num_rows($result) > 0) {
                    ...
                    // Redirect after successful login with additional security
                    header("Location: index.php?id=" . urlencode($user['idusers']));
                    exit();
                } else {
                    // Failed login
